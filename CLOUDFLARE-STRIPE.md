...
2. Store it as the Cloudflare Worker secret STRIPE_SECRET_KEY with `pnpm exec wrangler secret put STRIPE_SECRET_KEY --name zurtex`. Enter the key at the prompt. Never put it in client code or Git. For local testing, copy `.env.example` to ignored `.dev.vars` and add the test key there.
